...
export function LocalRegisterSwaggerDoc() {
    return applyDecorators(
        ApiOperation({
            ...
            description: "This function is responsible for handling the registration process for new students using local authentication. \
            Local authentication means using `username` and `password`."
        }),
        ApiBody({
            type: CreateStudentDto,
            description: 'The DTO object containing the student information for registration.',
        }),
        ApiResponse({ status: 201, description: 'The newly registered student object.' }),
...
export function localLoginSwaggerDoc() {
    return applyDecorators(
        ApiOperation({
            ...
            description: "This function is responsible for handling the login process for users using local authentication. \
            Local authentication means using `username` and `password`."
        }),
        ApiBody({
            schema: {
                ...
                properties: {
                    username: {
                        type: 'string',
                        description: "**WARNING**: name inconsistency. pass the `email` of the user.",
        ...
        ApiResponse({ status: 201, description: 'The user has been authenticated and a session id is successfully created.' }),
...
export function googleRedirectSwaggerDoc() {
    return applyDecorators(
        ApiOperation({
            summary: "Figures out what happens when Google redirects to the backend after a successful authentication.",
...
export function googleRegisterSwaggerDoc() {
    return applyDecorators(
        ...
        ApiResponse({ status: 201, description: 'The student has been sucessfuly created' }),
        ApiResponse({ status: 500, description: 'Internal Server Error' })
    );
}
...
export function getSessionUserSwaggerDoc() {
    return applyDecorators(
        ApiOperation({ summary: 'Gets the user information stored inside the session' }),
        ApiResponse({ status: 200, description: 'Returns the user information stored inside the session' }),
        ApiResponse({ status: 401, description: 'Unauthorized' })
    );
}
