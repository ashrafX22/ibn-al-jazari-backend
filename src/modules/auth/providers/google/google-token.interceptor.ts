...
export class GoogleTokenInterceptor implements NestInterceptor {
    ...
    async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<any>> {
        ...
        if (!googleAccessToken)
            throw new UnauthorizedException('No Google access token found in the user payload.');
        ...
        if (refreshedGoogleAccessToken) {
            ...
            console.log("GoogleTokenInterceptor refreshedGoogleAccessToken", refreshedGoogleAccessToken);
